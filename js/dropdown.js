		...
		return this.each(function(){
			...
			{'class':(settings.disabled?settings.disabledclass:settings.selectclass),
			 ...
			 'click':function(evt){
			 	...
			 	$('div.'+settings.listboxclass,$this).css({'top':'48px','left':'63px'}).show();
			 	stopBubble(evt);
			 }
			}).appendTo($this);
			...
			var listbox = $('<div>',{
							...
							'css': {'position':'absolute','width':'568px','overflow':'hidden','overflow-y':'auto','display':'none','z-index':1000}
						}).appendTo($this);
			if(settings.checkbox)
			{
			...
			objid.init = function(mdata,selectedValue){
				...
				selecttext = selecttext==''?'选择部门':selecttext;
